...
pwl.nn <- function(data, noOfBP = NULL, l, maxBP = NULL, ...){
  ...
  if(!is.null(noOfBP)){
    if(noOfBP > maxBP.allowed) stop("The data set is not big enough to fit the number of breakpoints given.\n Either lower the number of desired break points or distance between break points", call. = FALSE)
  }
  if(!is.null(maxBP)){
    if(maxBP > maxBP.allowed) stop("The data set is not big enough to fit the number of breakpoints given.\n Either lower the number of desired break points or distance between break points", call. = FALSE)
  }
