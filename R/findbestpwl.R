#' A function that is used to find the best set of pwl in that will return a mse% difference of less than specified error.
...
#' @return The set of pwl equations that satisfies the given error

# First, we try if all node with 2BP is sufficient.
# If not, we perform a sensitivity analysis (SA) by increaing the breakpoint one node at at a time and see if the best solution could be given
...
findbestpwl <- function(Inputdata, ANNOutput, ActualOutput, hiddenW, outputW, actfun="sigmoid", allpwlfull, error){
  ...
  basepwloutput <- predicts.pwlnn(Inputdata, ANNOutput, ActualOutput, hiddenW, outputW, basepwl, actfun="sigmoid")
  ...
  repeat{
    ...
    repeat{
      ...
      newpwloutput <- predicts.pwlnn(Inputdata, ANNOutput, ActualOutput, hiddenW, outputW, newpwl, actfun="sigmoid")
  ...
  if(ncol(FilterSA)==1){
    ...
    combopwloutput <- predicts.pwlnn(Inputdata, ANNOutput, ActualOutput, hiddenW, outputW, combopwl, actfun="sigmoid")
    ...
    if(combomseper < error){
      ...
      repeat{
        ...
        repeat{
          ...
          nextcombopwloutput <- predicts.pwlnn(Inputdata, ANNOutput, ActualOutput, hiddenW, outputW, nextcombopwl, actfun="sigmoid")
